    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-header">
                {{ trans('global.create') }} {{ trans('cruds.sales.title_singular') }}
            </div>
            ...
            <div class="card-body">
                <form action="{{ route('admin.sales.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group {{ $errors->has('cash') ? 'has-error' : '' }}">
                        ...
                        <input type="number" id="cash" name="cash" class="form-control" value="{{ old('cash') }}" step="0.01" required onkeyup="calCashBalance();">
                        @if($errors->has('cash'))
                            <em class="invalid-feedback">
                                {{ $errors->first('cash') }}
                            </em>
                    ...
                    <div class="form-group {{ $errors->has('pay_out') ? 'has-error' : '' }}">
                        ...
                        <input type="number" id="pay_out" name="pay_out" class="form-control" value="{{ old('pay_out') }}" step="0.01" required onkeyup="calCashBalance();">
                    ...
                    <div class="form-group {{ $errors->has('cash_balance') ? 'has-error' : '' }}">
                        ...
                        <input type="number" id="cash_balance" name="cash_balance" class="form-control" value="{{ old('cash_balance') }}" step="0.01" required readonly>
                    ...
                    <div class="form-group {{ $errors->has('card') ? 'has-error' : '' }}">
                        ...
                        <input type="number" id="card" name="card" class="form-control" value="{{ old('card') }}" step="0.01" required>
                        @if($errors->has('card'))
                            <em class="invalid-feedback">
                                {{ $errors->first('card') }}
                            </em>
