    ...
    <div class="row page-titles mx-0">
        <div class="col p-md-0">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">{{ trans('global.create') }} {{ trans('cruds.vendor.title_singular') }}</li>
            </ol>
        </div>
    </div>
            ...
            <div class="card-body">
                <form action="{{ route('admin.vendors.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                        <label for="name">{{ trans('cruds.vendor.fields.name') }}*</label>
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', isset($vendor) ? $vendor->name : '') }}" required>
                        @if($errors->has('name'))
